...
- [3. 饿汉式](#3-饿汉式)
- [4. 普通懒汉式](#4-普通懒汉式)
- [5. synchronized 懒汉式](#5-synchronized-懒汉式)
- [6. DCL 双重检查锁](#6-dcl-双重检查锁)
- [7. 静态内部类](#7-静态内部类)
- [8. 枚举单例](#8-枚举单例)
- [9. Spring 单例 Bean](#9-spring-单例-bean)
- [10. 单例和线程安全](#10-单例和线程安全)
- [11. 几种实现方式对比](#11-几种实现方式对比)
- [12. 当前项目中哪里使用到了](#12-当前项目中哪里使用到了)
  - [12.1 显式饿汉式工具单例：`CornexSignUtils`](#121-显式饿汉式工具单例cornexsignutils)
  - [12.2 Spring 单例配置 Bean：线程池统一复用](#122-spring-单例配置-bean线程池统一复用)
  - [12.3 Spring 单例客户端 Bean：`RedissonClient`](#123-spring-单例客户端-beanredissonclient)
  - [12.4 Spring 单例策略工厂：`WorkOrderStrategyFactory`](#124-spring-单例策略工厂workorderstrategyfactory)
  - [12.5 Spring 生命周期钩子：`SmartInitializingSingleton`](#125-spring-生命周期钩子smartinitializingsingleton)
  - [12.6 Spring 单例工具 Bean：`KafkaUtil`、`RedisLockUtil`](#126-spring-单例工具-beankafkautilredislockutil)
- [13. 实际工作怎么判断是否该用单例](#13-实际工作怎么判断是否该用单例)
- [14. 复习的思路](#14-复习的思路)
- [15. 面试讲解思路](#15-面试讲解思路)
- [16. 面试时可以这样说](#16-面试时可以这样说)
- [17. 参考来源](#17-参考来源)
