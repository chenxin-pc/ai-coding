...
- [1. 它是什么](#1-它是什么)
- [2. 为什么需要工厂模式](#2-为什么需要工厂模式)
- [3. 简单工厂](#3-简单工厂)
- [4. 工厂方法](#4-工厂方法)
- [5. 抽象工厂](#5-抽象工厂)
- [6. Spring 和工厂模式](#6-spring-和工厂模式)
- [7. 工厂模式和策略模式的关系](#7-工厂模式和策略模式的关系)
- [8. 当前项目中哪里使用到了](#8-当前项目中哪里使用到了)
  - [8.1 `LoginFactory`：简单工厂 + 注册表](#81-loginfactory简单工厂-注册表)
  - [8.2 `InvokerFactory`：工厂组装执行链](#82-invokerfactory工厂组装执行链)
  - [8.3 `OrderFactoryService`：按客户平台选择下单实现](#83-orderfactoryservice按客户平台选择下单实现)
  - [8.4 `StrategyFactory`：重试推送策略工厂](#84-strategyfactory重试推送策略工厂)
  - [8.5 `OpenApiCallerCacheService`：按 `apiCode` 查找缓存处理器](#85-openapicallercacheservice按-apicode-查找缓存处理器)
  - [8.6 `@Bean` 工厂方法：数据源、线程池、HTTP 客户端](#86-bean-工厂方法数据源线程池http-客户端)
- [9. 实际工作中的使用建议](#9-实际工作中的使用建议)
- [10. 复习的思路](#10-复习的思路)
- [11. 面试讲解思路](#11-面试讲解思路)
- [12. 面试时可以这样说](#12-面试时可以这样说)
- [13. 参考来源](#13-参考来源)
