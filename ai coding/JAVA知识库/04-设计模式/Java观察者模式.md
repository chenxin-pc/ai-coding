...
- [3. 基础代码示例](#3-基础代码示例)
- [4. 核心原理](#4-核心原理)
- [5. JDK 自带 Observer 为什么不推荐](#5-jdk-自带-observer-为什么不推荐)
- [6. Spring Event 实现观察者模式](#6-spring-event-实现观察者模式)
  - [6.1 定义事件](#61-定义事件)
  - [6.2 发布事件](#62-发布事件)
  - [6.3 监听事件](#63-监听事件)
- [7. MQ 实现分布式观察者模式](#7-mq-实现分布式观察者模式)
- [8. 项目中哪里使用到了](#8-项目中哪里使用到了)
  - [8.1 gateway-server：Spring 本地事件](#81-gateway-serverspring-本地事件)
  - [8.2 ka-solution：电子签回调通知](#82-ka-solution电子签回调通知)
  - [8.3 ka-solution：电子签短信通知](#83-ka-solution电子签短信通知)
  - [8.4 ka-solution：电子签报告文件延迟获取](#84-ka-solution电子签报告文件延迟获取)
  - [8.5 ka-solution：网点信息监听](#85-ka-solution网点信息监听)
  - [8.6 ka-monitor：告警跟进和工单延迟上报](#86-ka-monitor告警跟进和工单延迟上报)
  - [8.7 ka-retry：公共重试服务](#87-ka-retry公共重试服务)
  - [8.8 ka-common：Rabbit Listener 基础设施](#88-ka-commonrabbit-listener-基础设施)
  - [8.9 需要避免误判的 Subscribe 命名](#89-需要避免误判的-subscribe-命名)
- [9. 实际工作中怎么判断该不该用](#9-实际工作中怎么判断该不该用)
- [10. 生产注意事项](#10-生产注意事项)
  - [10.1 同步观察者异常隔离](#101-同步观察者异常隔离)
  - [10.2 异步事件要考虑事务时机](#102-异步事件要考虑事务时机)
  - [10.3 MQ 监听要默认考虑重复消费](#103-mq-监听要默认考虑重复消费)
  - [10.4 Listener 里不要写无限重试](#104-listener-里不要写无限重试)
  - [10.5 Topic 和 Consumer Group 要设计清楚](#105-topic-和-consumer-group-要设计清楚)
- [11. 和其他模式的区别](#11-和其他模式的区别)
- [12. 复习的思路](#12-复习的思路)
- [13. 面试讲解思路](#13-面试讲解思路)
- [14. 一句话总结](#14-一句话总结)
- [15. 参考资料](#15-参考资料)
